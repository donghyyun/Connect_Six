...
public class SelectColor extends JPanel implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == black) {
			if(!selected) {
				int ans = JOptionPane.showConfirmDialog(null, "흑돌로 게임하시겠습까?" , "알림", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null);
				if(ans == 0) {
				...
				JOptionPane.showMessageDialog(null, "돌을 이미 선택했습니다!", "오류", JOptionPane.ERROR_MESSAGE);

		}
		
		if(e.getSource() == white) {
			if(!selected) {
				int ans = JOptionPane.showConfirmDialog(null, "백돌로 게임하시겠습까?" , "알림", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null);
				if(ans == 0) {
