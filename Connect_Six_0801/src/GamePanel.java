...
public class GamePanel extends JPanel{
	...
	private void drawlaststones(Graphics g) {
		...
		if (!Turn().equals(ComColor)) {
			...
			g2.drawOval(userL.get(userL.size() -1).x - 21, userL.get(userL.size() -1).y -21, Stone.STONE_SIZE + 1, Stone.STONE_SIZE + 1);
			g2.drawOval(userL.get(userL.size() -2).x - 21, userL.get(userL.size() -2).y -21, Stone.STONE_SIZE + 1, Stone.STONE_SIZE + 1);
		}
	}
